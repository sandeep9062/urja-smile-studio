  ...
  if (!service) {
    return (
      <div className="py-32 text-center">
        ...
        <Link href="/services" className="mt-4 inline-block text-primary underline">
          Back to services
        </Link>
      </div>
    );
    ...
    <form onSubmit={onSubmit} className="rounded-2xl border border-border bg-card p-6 shadow-soft">
      ...
      <div className="mt-5 space-y-3">
        <div>
          ...
          <Input id="cf-name" name="name" required maxLength={100} placeholder="Your full name" />
        </div>
        <div>
          <Label htmlFor="cf-phone">Phone</Label>
          <Input id="cf-phone" name="phone" type="tel" required placeholder="+91 98xxx xxxxx" />
        </div>
        <div>
          <Label htmlFor="cf-service">Service</Label>
          <Input id="cf-service" name="service" defaultValue={serviceTitle} readOnly />
        </div>
        <div>
        ...
        <div>
          <Label htmlFor="cf-note">Brief note (optional)</Label>
          <Textarea id="cf-note" name="note" rows={3} maxLength={300} />
        </div>
