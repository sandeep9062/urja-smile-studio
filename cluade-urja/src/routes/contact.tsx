...
export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Get in touch — call, WhatsApp, email or visit our clinic on MG Road, Bengaluru." },
    ],
  }),
...
function ContactPage() {
  ...
  const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setSending(true);
    setTimeout(() => { setSending(false); toast.success("Message sent! We'll respond within one business day."); form.reset(); }, 700);
  };

  return (
    <>
      <PageHeader title="Contact Us" subtitle="We'd love to hear from you." crumbs={[{ label: "Contact" }]} />
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 grid lg:grid-cols-2 gap-10">
        <div className="space-y-8">
          {[
            ...
            { icon: MessageCircle, title: "WhatsApp", body: "Chat with our team", href: CLINIC.whatsapp },
            ...
            <div key={title} className="flex gap-4">
              <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-accent text-primary"><Icon className="h-5 w-5" /></div>
              <div>
                <p className="font-semibold">{title}</p>
                {href ? <a href={href} className="text-sm text-muted-foreground hover:text-primary">{body}</a> : <p className="text-sm text-muted-foreground">{body}</p>}
              </div>
            </div>
          ))}

          <div className="rounded-2xl overflow-hidden border border-border h-72 bg-muted">
            <iframe title="Google Maps" src="https://www.google.com/maps?q=MG+Road+Bengaluru&output=embed" className="w-full h-full" loading="lazy" />
          </div>
        </div>

        <form onSubmit={onSubmit} className="rounded-3xl border border-border bg-card p-6 md:p-10 shadow-soft space-y-4 h-fit">
          ...
          <div><Label htmlFor="subject">Subject</Label><Input id="subject" name="subject" maxLength={150} /></div>
