...
export const Route = createFileRoute("/testimonials")({
  head: () => ({
    meta: [
      { title: "Patient Testimonials — Urja Dental Clinic" },
      { name: "description", content: "Hear from 25,000+ patients who trusted Urja Dental for their smiles." },
    ],
  }),
...
function TestimonialsPage() {
  return (
    <>
      <PageHeader title="Patient Testimonials" subtitle="Stories from the smiles we've cared for." crumbs={[{ label: "Testimonials" }]} />
      ...
      <section className="bg-card border-y border-border">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
          ...
          <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {[1, 2, 3].map((i) => (
              <div key={i} className="relative rounded-2xl overflow-hidden border border-border group cursor-pointer">
                <img src={`https://picsum.photos/seed/video-${i}/600/400`} alt={`Video testimonial ${i}`} className="h-56 w-full object-cover" loading="lazy" />
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="mt-6 rounded-2xl border border-dashed border-border p-12 bg-card text-center">
          ...
          <p className="mt-2 text-2xl font-bold">4.9 <span className="text-coral">★★★★★</span></p>
