...
export const Route = createFileRoute("/social")({
  head: () => ({
    meta: [
      { title: "Follow Us — Urja Dental Clinic" },
      { name: "description", content: "Follow @urjadentalclinic on Instagram for smile tips, patient stories and behind-the-scenes." },
    ],
  }),
...
function SocialPage() {
  return (
    <>
      <PageHeader title="Follow Us on Instagram" subtitle="@urjadentalclinic — smile tips, real transformations and clinic life." crumbs={[{ label: "Social" }]} />
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-10">
          ...
          <Button asChild><a href="https://instagram.com" target="_blank" rel="noreferrer"><Instagram className="mr-2 h-4 w-4" /> Follow on Instagram</a></Button>
        </div>

        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 md:gap-4">
          {POSTS.map((p) => (
            <article key={p.id} className="group relative rounded-2xl overflow-hidden border border-border bg-card aspect-square">
              <img src={p.image} alt={p.caption} className="h-full w-full object-cover group-hover:scale-105 transition duration-500" loading="lazy" />
              <div className="absolute inset-0 bg-foreground/60 opacity-0 group-hover:opacity-100 transition flex flex-col items-center justify-center text-background p-4 text-center">
                <div className="flex gap-5 text-sm font-semibold">
                  <span className="flex items-center gap-1"><Heart className="h-4 w-4" fill="currentColor" /> {p.likes}</span>
                  <span className="flex items-center gap-1"><MessageCircle className="h-4 w-4" fill="currentColor" /> {p.comments}</span>
                </div>
